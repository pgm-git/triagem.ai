        ...
        <div className="space-y-6 pb-20">
            <div className="flex items-center justify-between">
                ...
                <button
                    ...
                    className="flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white text-sm font-medium rounded-lg transition-all cursor-pointer"
                >
            ...
            <div className="space-y-4">
                {/* Entry point */}
                <div className="flex items-center gap-3 py-3 px-4 bg-slate-900 border border-slate-800 rounded-xl">
                    <div className="w-10 h-10 rounded-xl bg-emerald-500/10 flex items-center justify-center">
                        <GitBranch className="w-5 h-5 text-emerald-400" />
                    </div>
                    <div>
                        <p className="text-sm font-semibold text-white">Mensagem Recebida do Canal WhatsApp</p>
                        <p className="text-xs text-slate-500">A mensagem entra no Pipeline de Avaliação</p>
                    </div>
                </div>
                ...
                {/* --- LAYER 1: KEYWORDS --- */}
                <div className="p-5 border border-slate-800 bg-slate-900/50 rounded-2xl">
                    <div className="flex items-center justify-between mb-4">
                        <h2 className="text-sm font-semibold text-white uppercase tracking-wider text-slate-400 text-[11px] mb-2">Camada 1: Correspondência Exata (Palavras-Chave)</h2>
                        <span className="text-[10px] bg-slate-800 text-slate-400 px-2 py-0.5 rounded-full">{totalKeywords} Gatilhos Ativos</span>
                    </div>

                    <div className="space-y-3">
                        {regularSectors.map((sector) => (
                            <div key={sector.id} className="border border-blue-500/20 bg-blue-500/5 rounded-xl p-4">
                                ...
                                <div className="flex flex-wrap gap-1.5 mb-2">
                                    ...
                                    )) : <span className="text-xs text-slate-500">Sem palavras-chave definidas</span>}
                                </div>
                                <div className="flex items-start gap-2 pt-2 border-t border-blue-500/10">
                                    <ArrowRight className="w-3.5 h-3.5 text-blue-400 mt-0.5 shrink-0" />
                                    <p className="text-xs text-slate-400">Resposta automática: "{sector.response_template || 'Nenhuma'}"</p>
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                <div className="p-5 border border-purple-500/20 bg-purple-500/5 rounded-2xl relative overflow-hidden">
                    ...
                    <div className="relative z-10">
                        ...
                        <div className="bg-slate-900 border border-slate-800 rounded-lg p-4 max-w-md">
                            ...
                            <p className="text-xs text-slate-400 mt-1 line-clamp-2">{activePersona?.description || 'Nenhuma descrição fornecida'}</p>
                            <div className="mt-3 pt-3 border-t border-slate-800">
                                <p className="text-[10px] font-medium text-purple-400 uppercase tracking-wider mb-1">Contexto Global (Empresa)</p>
                                <p className="text-xs text-slate-400 line-clamp-2 italic">"{aiSettings?.custom_prompt_base || 'Sem contexto adicional configurado'}"</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                {fallbackSector && (
                    <div className="flex items-center gap-3 py-3 px-4 bg-amber-500/5 border border-amber-500/20 rounded-xl">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-slate-500">Para onde vão as conversas não classificadas ou com erro na IA</p>
                        </div>
                    </div>
                )}
            </div>
                ...
                <>
                    ...
                    <div className="fixed right-0 top-0 h-screen w-full max-w-lg bg-slate-950 border-l border-slate-800 z-50 flex flex-col animate-in slide-in-from-right duration-200">
                        <div className="p-5 border-b border-slate-800 flex items-center justify-between">
                            ...
                            <button onClick={() => setShowAiDrawer(false)} className="text-slate-500 hover:text-white"><X className="w-5 h-5" /></button>
                        </div>
                        <div className="flex-1 overflow-y-auto p-5 space-y-6">
                            <div>
                                ...
                                <div className="space-y-4">
                                    <div>
                                        <label className="text-xs text-slate-400 mb-1 block">Nome do Arquétipo</label>
                                        <input
                                    ...
                                    <div>
                                        <label className="text-xs text-slate-400 mb-1 block">Descrição do Perfil</label>
                                        <input
                                    ...
                                    <div>
                                        <label className="text-xs text-slate-400 mb-1 block">Instruções Comportamentais Restritas (Como deve falar?)</label>
                                        <textarea
                                            ...
                                            className="w-full bg-slate-900 border border-slate-800 rounded-lg px-3 py-2 text-sm text-white resize-none"
                                        />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="border-t border-slate-800 pt-6">
                                ...
                                <div>
                                    <label className="text-xs text-slate-400 mb-1 block">Informações que a IA deve saber</label>
                                    <textarea
                                        ...
                                        className="w-full bg-slate-900 border border-slate-800 rounded-lg px-3 py-2 text-sm text-white resize-none"
                                    />
                                    <p className="text-[10px] text-slate-500 mt-1">Este contexto é injetado junto com as informações da Persona quando o WhatsApp envia a mensagem para a IA avaliar.</p>
                                </div>
                            </div>
                        </div>
                        <div className="p-5 border-t border-slate-800 bg-slate-950 flex gap-3">
                            <button onClick={() => setShowAiDrawer(false)} className="flex-1 py-2 rounded-lg bg-slate-800 text-sm text-white font-medium hover:bg-slate-700">Cancelar</button>
                            <button
                                onClick={handleSaveAiSettings} disabled={isSaving}
                                className="flex-1 py-2 rounded-lg bg-blue-600 text-sm text-white font-medium hover:bg-blue-500 flex items-center justify-center gap-2"
                            >
