...
export default function ConfiguracoesPage() {
    ...
    const [personaTraits, setPersonaTraits] = useState<PersonaTraitId[]>(['professional', 'welcoming']);
    ...
    useEffect(() => {
        if (organization) {
            ...
            if (organization.persona_traits) {
                setPersonaTraits(organization.persona_traits as PersonaTraitId[]);
            }
        }
    }, [organization]);
    ...
    const handleSavePersona = async () => {
        ...
        try {
            const res = await fetch('/api/organizations', {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ persona_traits: personaTraits })
            });
            if (!res.ok) throw new Error('Falha ao salvar');
                        ...
                        <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
                            <div className="flex items-center justify-between mb-6">
                                ...
                                <button
                                    ...
                                    disabled={saving}
                                    className="flex items-center gap-2 px-4 py-2 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white text-xs font-medium rounded-lg transition-all cursor-pointer"
                                >
                                    {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}
                                    Salvar Persona
                                </button>
                            </div>
                            <PersonaConfig value={personaTraits} onChange={setPersonaTraits} />
                        </div>
                    </div>
                )}
