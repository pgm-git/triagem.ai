...
const colorByIndex: Record<number, { dot: string; border: string; bg: string; text: string }> = {
    0: { dot: 'bg-indigo-500', border: 'border-indigo-500/20', bg: 'bg-indigo-500/5', text: 'text-indigo-400' },
...
export function SectorCard({ sector, onEdit, onToggle, onDelete }: SectorCardProps) {
    ...
    return (
        <div
            className={cn(
                'bg-zinc-900 border rounded-xl transition-all hover:scale-[1.005]',
                sector.is_active ? colors.border : 'border-zinc-800 opacity-60'
            )}
        >
            {/* Header */}
            <div className="flex items-center justify-between p-4">
                <div className="flex items-center gap-3 min-w-0">
                    <div className="w-10 h-10 rounded-xl bg-zinc-800 flex items-center justify-center text-xl shrink-0">
                        {sector.icon || <Building2 className="w-5 h-5 text-zinc-400" />}
                    </div>
                    <div className="min-w-0">
                        <h3 className="text-sm font-semibold text-white truncate">{sector.name}</h3>
                        <p className="text-[11px] text-zinc-500 truncate">{sector.destination}</p>
                    </div>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                    ...
                    <button
                        ...
                        className={cn(
                            ...
                            sector.is_active ? 'bg-indigo-600 justify-end' : 'bg-zinc-700 justify-start'
                        )}
                    >
                    ...
                    <div className="relative">
                        ...
                        {menuOpen && (
                            <>
                                ...
                                <div className="absolute right-0 top-full mt-1 bg-zinc-900 border border-zinc-800 rounded-lg shadow-xl z-20 min-w-[140px] py-1">
                                    <button onClick={() => { onEdit(sector); setMenuOpen(false); }} className="flex items-center gap-2 w-full px-3 py-2 text-sm text-zinc-300 hover:bg-zinc-800 cursor-pointer">
                                        <Pencil className="w-3.5 h-3.5" /> Editar
                                    </button>
                                    <button onClick={() => { onDelete(sector.id); setMenuOpen(false); }} className="flex items-center gap-2 w-full px-3 py-2 text-sm text-red-400 hover:bg-zinc-800 cursor-pointer">
                ...
                <div className="px-4 pb-3">
                    <div className="flex flex-wrap gap-1.5">
                        ...
                        {!expanded && keywords.length > 4 && (
                            <span className="px-2 py-0.5 text-[10px] font-medium bg-zinc-800 text-zinc-500 rounded-md">
                                +{keywords.length - 4}
                            </span>
                        )}
                    </div>
            ...
            {response && (
                <div className="px-4 pb-3">
                    <div className="flex items-start gap-1.5">
                        ...
                        <p className="text-[11px] text-zinc-500 italic truncate">"{response}"</p>
                    </div>
                </div>
            )}
            ...
            <div className="flex items-center gap-1.5 flex-wrap px-4 pb-4">
                ...
                {keywords.length > 4 && (
                    <button
                        ...
                        className="flex items-center gap-0.5 px-2 py-0.5 text-[10px] font-medium text-zinc-500 hover:text-zinc-300 cursor-pointer transition-colors"
                    >
